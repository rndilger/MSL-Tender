"""
Analyze and fix Study 2104 mismatches
"""
import pandas as pd
import os
...
print("="*80)
print("STUDY 2104 ANALYSIS")
print("="*80)

df = pd.read_csv(csv_path)
study_2104 = df[df['study_number'] == 2104].copy()

print(f"\nDatabase records for Study 2104: {len(study_2104)}")

# Get images
image_chop_ids = set()
for root, dirs, files in os.walk(photos_dir):
    for file in files:
        if file.startswith('2104') and file.lower().endswith(('.jpg', '.jpeg', '.png')):
            filename_base = Path(file).stem
            image_chop_ids.add(filename_base)

print(f"Images found for Study 2104: {len(image_chop_ids)}")

# Compare
db_ids = set(study_2104['standardized_chop_id'].dropna())
...
df_cleaned = df[~((df['study_number'] == 2104) & ~df['standardized_chop_id'].isin(image_chop_ids))]
final_count = len(df_cleaned)

print(f"\nTotal records: {initial_count} → {final_count}")
print(f"Study 2104 records: {len(study_2104)} → {len(df_cleaned[df_cleaned['study_number'] == 2104])}")
print(f"Records removed: {initial_count - final_count}")

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
backup_path = os.path.join(project_root, "database", f"consolidated_data_before_2104_cleanup_{timestamp}.csv")
